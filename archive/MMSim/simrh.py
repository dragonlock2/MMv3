...
# Simulates mouse with sensors on front, left, and right
...
class Karel():
	...
	def openLeft(self):
		return not Maze.getWalls(self.maze.maze[self.x][self.y])[(self.dir-1)%4]
	...
	def gather(self): # note Arduino mod leaves negatives so need to fix that
		...
		if not self.openLeft():
			leftDir = (self.dir - 1) % 4
			self.recon[self.x][self.y] |= myUpdates[leftDir]
			n = neighs[leftDir]
			if self.isValid(n):
				self.recon[n[0]][n[1]] |= neighUpdates[leftDir]
		if not self.openRight():
			rightDir = (self.dir + 1) % 4
			self.recon[self.x][self.y] |= myUpdates[rightDir]
			n = neighs[rightDir]
			if self.isValid(n):
				self.recon[n[0]][n[1]] |= neighUpdates[rightDir]
	...
	def floodFillUpdate(self, toCenter):
		currentLevel, nextLevel = [], [] # some stacks, should optimize this
